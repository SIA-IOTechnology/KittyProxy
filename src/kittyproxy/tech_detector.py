...
class TechnologyDetector:
    ...
    SIGNATURES = {
        ...
        'security': {
            ...
            # Éviter le mot isolé "firebase" (trop de faux positifs dans le HTML marketing).
            'Firebase Hosting': [
                r'x-firebase',
                r'firebasehosting\.googleapis\.com',
                r'__FIREBASE_DEFAULTS__',
                r'firebase-app-check',
                r'initializeApp\s*\(\s*\{\s*apiKey',
                r'\.firebaseapp\.com',
                r'\.web\.app\b',
            ],
            'Firestore / Firebase API': [
                ...
                r'fcmregistrations\.googleapis\.com',
            ],
    ...
    def detect(self, flow) -> Dict[str, List[str]]:
        ...
        self._apply_host_technology_hints(hostname, scores)
    ...
    def _apply_host_technology_hints(self, hostname: str, scores: Dict[str, Dict[str, int]]) -> None:
        """Bonus score basé sur le hostname (Firebase Hosting, APIs Google liées à Firebase)."""
        if not hostname:
            return
        h = str(hostname).lower().strip(".")
        if h.endswith(".web.app") or h.endswith(".firebaseapp.com"):
            scores["security"].setdefault("Firebase Hosting", 0)
            scores["security"]["Firebase Hosting"] += 3
        if h.endswith(".firebaseio.com") or "firebasedatabase.app" in h:
            scores["security"].setdefault("Firebase Realtime Database", 0)
            scores["security"]["Firebase Realtime Database"] += 3
        firebase_api_hosts = (
        ...
        if h in firebase_api_hosts:
            scores["security"].setdefault("Firestore / Firebase API", 0)
            scores["security"]["Firestore / Firebase API"] += 3
