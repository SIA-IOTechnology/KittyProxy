...
def execute_module_from_flow(request: Dict):
    ...
    try:
        ...
        auto_options['http_method'] = flow.request.method or 'GET'

        # Captured response for credential-leak scanners
        auto_options.update(_proxy_flow_options(flow_id))
...
def _proxy_flow_options(flow_id: str) -> Dict:
    """Build module options from a captured KittyProxy flow (response body + method)."""
    opts = {}
    if not flow_id:
        return opts
    flow_data = flow_manager.get_flow(flow_id)
    if not flow_data:
        return opts
    opts["proxy_flow_id"] = flow_id
    resp = flow_data.get("response") or {}
    if resp.get("content_bs64"):
        opts["response_body_b64"] = resp["content_bs64"]
    method = flow_data.get("method") or "GET"
    opts["http_method"] = method
    opts["METHOD"] = method
    try:
        from urllib.parse import urlparse
        parsed = urlparse(flow_data.get("url") or "")
...
def run_module(request: Dict):
    ...
    try:
        ...
        # Inject captured response from proxy flow when requested
        proxy_flow_id = options.pop('proxy_flow_id', None) or options.pop('PROXY_FLOW_ID', None)
        if proxy_flow_id:
            options.update(_proxy_flow_options(proxy_flow_id))
...
# Serve logo and favicon from framework interfaces/static/img/
# These routes must be defined before the static files mount to take precedence
from ._paths import shared_static_img_dir, browser_icons_dir as _browser_icons_dir, framework_root

static_img_dir = str(shared_static_img_dir())
...
browser_icons_dir = str(_browser_icons_dir())
