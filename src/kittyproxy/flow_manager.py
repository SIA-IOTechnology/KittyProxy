...
class FlowManager:
    def __init__(self, max_flows: int = 5000, fast_mode: bool = False, fast_mode_threshold_kb: int = 100, framework=None):
        ...
        self.flow_body_cache: Dict[str, Dict] = {}  # request/response bodies (base64), built in background
        ...
        self.body_worker = threading.Thread(target=self._body_cache_worker, daemon=True)
    ...
    def add_flow(self, flow):
        ...
        with self._lock:
            ...
            if len(self.flows) > self.max_flows:
                ...
                if oldest_id in self.flow_body_cache:
                    del self.flow_body_cache[oldest_id]
            
            serialized_flow = self._serialize_flow_minimal(flow)
            self.flow_cache[flow.id] = serialized_flow

            if flow.response:
                self._queue_body_cache(flow.id)
    ...
    def get_flow(self, flow_id: str) -> Optional[Dict]:
        """Returns full details of a specific flow (fast: no synchronous heavy analysis)."""
        with self._lock:
            ...
            if not flow:
                return None
            bodies = self.flow_body_cache.get(flow_id)
            flow_ref = flow
            queue_analysis = bool(flow.response and flow_id not in self.flow_analysis_cache)

        if bodies is None:
            bodies = self._serialize_flow_bodies(flow_ref)
            with self._lock:
                if flow_id not in self.flow_body_cache:
                    self.flow_body_cache[flow_id] = bodies

        with self._lock:
            if queue_analysis:
                try:
                    self.analysis_queue.put_nowait((flow_id, flow_ref))
                except queue.Full:
                    pass
            minimal_data = self.flow_cache.get(flow_id, {})
            analysis_data = self.flow_analysis_cache.get(flow_id) or {
                "technologies": minimal_data.get("technologies") or {},
                "fingerprint": minimal_data.get("fingerprint") or {},
                "module_suggestions": minimal_data.get("module_suggestions") or [],
                "endpoints": minimal_data.get("endpoints") or {},
                "discovered_endpoints": minimal_data.get("discovered_endpoints") or [],
                "ssrf_redirect_candidates": minimal_data.get("ssrf_redirect_candidates") or [],
            }
            result = {**minimal_data, **analysis_data}
            result["request"] = bodies["request"]
    ...
    def _serialize_flow_headers_only(self, flow) -> Dict:
        """Request/response metadata for list view and instant detail headers (no body)."""
        req = {
        ...
        if flow.response:
            res = {
                ...
                "content_length": _safe_response_size(flow.response) or 0,
    ...
    def _serialize_flow_bodies(self, flow) -> Dict:
        ...
        if flow.response:
            ...
            bodies["response"] = None
        return bodies

    def _queue_body_cache(self, flow_id: str) -> None:
        if flow_id in self.flow_body_cache:
            return
        try:
            self.body_queue.put_nowait(flow_id)
        except queue.Full:
            pass

    def _body_cache_worker(self) -> None:
        while self.analysis_worker_running:
            try:
                flow_id = self.body_queue.get(timeout=1)
            except queue.Empty:
                continue
            try:
                with self._lock:
                ...
                if not flow:
                    continue
                bodies = self._serialize_flow_bodies(flow)
                with self._lock:
                    if flow_id not in self.flow_body_cache:
                        self.flow_body_cache[flow_id] = bodies
            except Exception as e:
                print(f"[ERROR] Body cache worker: {e}")
            finally:
                try:
    ...
    def _serialize_flow_minimal(self, flow) -> Dict:
        """Minimal serialization: method, URL, status, headers (no body base64)."""
        duration = None
        duration_ms = None
        ...
        hdr_parts = self._serialize_flow_headers_only(flow)
        return {
            "id": flow.id,
            ...
            "request": hdr_parts["request"],
            "response": hdr_parts["response"],
    ...
    def _perform_heavy_analysis(self, flow) -> Dict:
        ...
        try:
            ...
            url_categories = (
                'html_links', 'form_actions', 'javascript_endpoints', 'json_urls',
                'css_urls', 'api_endpoints', 'other_resources', 'react_api_endpoints',
                'image_urls',
            )
            total_extracted = sum(
                len(extracted_endpoints.get(cat) or [])
        ...
        # Credential / secret exposure in HTTP response
        try:
            response_secrets = extracted_endpoints.get('response_secrets') or []
            if response_secrets:
                fingerprint.setdefault('vulnerabilities', [])
                flow_url = flow.request.url if flow.request else ''
                for secret in response_secrets:
                    ...
                    fingerprint['vulnerabilities'].append({
                        ...
                        'severity': severity,
                        'description': f"Exposed {secret_type}: {secret_name}",
                        'type': 'credential_exposure',
                        'secret_type': secret_type,
                        'source_url': secret.get('source_url') or flow_url,
                    })

                reasons = [
                    f"Exposed {s.get('type', 'secret')}: {s.get('name', '')}"
                    for s in response_secrets[:3]
                ]
                credential_modules = [
                    'scanner/http/http_response_credential_leak_detect',
                ]
                security_techs = detected_techs.get('security', []) or []
                flow_host = (flow.request.host or '').lower() if flow.request else ''
                # RTDB scanner only when host/tech is Realtime Database — not Firestore API leaks
                if (
                    'Firebase Realtime Database' in security_techs
                    or '.firebaseio.com' in flow_host
                    or 'firebasedatabase.app' in flow_host
                ):
                    credential_modules.append('scanner/http/firebase_rtdb_public_access_detect')

                for idx, module_path in enumerate(reversed(credential_modules)):
                    if any(sug.get('module') == module_path for sug in module_suggestions):
                        continue
                    module_suggestions.insert(0, {
                        'module': module_path,
                        'score': 80 - (idx * 15),
                        'reasons': reasons,
                        'priority': 'high',
            ...
            print(f"[ERROR] Error processing response secrets: {e}")
        
        # Ajouter aux endpoints découverts globaux
        try:
            url_categories = (
                'html_links', 'javascript_endpoints', 'api_endpoints',
                'react_api_endpoints', 'json_urls', 'css_urls', 'other_resources',
            )
            for category in url_categories:
                for url in extracted_endpoints.get(category) or []:
        ...
        return {
            ...
            "discovered_endpoints": sorted(list(set([
                url for cat in (
                    'html_links', 'javascript_endpoints', 'api_endpoints',
                    'react_api_endpoints', 'json_urls', 'css_urls', 'other_resources',
                )
                for url in (extracted_endpoints.get(cat) or [])
    ...
    def remove_flows_out_of_scope(self) -> int:
        ...
        with self._lock:
            ...
            for flow_id in flows_to_remove:
                ...
                if flow_id in self.flow_body_cache:
                    del self.flow_body_cache[flow_id]
