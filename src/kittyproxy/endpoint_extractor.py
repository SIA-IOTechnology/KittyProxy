...
class EndpointExtractor:
    ...
    def __init__(self, framework=None):
        ...
        # Captured response bodies by flow_id (base64, truncated) for Secrets modal
        self.flow_response_bodies: Dict[str, str] = {}
        self._max_response_body_capture = 400_000
    ...
    def reset(self):
        ...
        self.flow_response_bodies.clear()
    ...
    # Strict PEM private key block detection to reduce false positives.
    # We require both BEGIN + END markers and a non-trivial payload between them.
    ...
    _GCP_PRIVATE_KEY_JSON = re.compile(
        r'(?:\\?"private_key\\?"|["\']private_key["\'])\s*:\s*(?:\\?"|["\']).*?-----BEGIN',
        re.IGNORECASE | re.DOTALL,
    )
    ...
    def _secret_dedup_key(self, secret: Dict) -> tuple:
        return (
            secret.get('source_url') or '',
            secret.get('type') or '',
            (secret.get('name') or '').lower(),
        )

    def _cache_response_body_for_flow(self, flow_id: str, content: str) -> None:
        if not flow_id or not content:
            return
        truncated = content[: self._max_response_body_capture]
        self.flow_response_bodies[flow_id] = base64.b64encode(
            truncated.encode("utf-8", errors="replace")
        ).decode("ascii")

    def _enrich_secret_with_capture(self, secret: Dict) -> Dict:
        flow_id = secret.get("flow_id")
        if flow_id and flow_id in self.flow_response_bodies:
            secret["response_body_b64"] = self.flow_response_bodies[flow_id]
        return secret

    def _register_discovered_secret(self, secret: Dict) -> bool:
        secret = self._enrich_secret_with_capture(secret)
        key = self._secret_dedup_key(secret)
        for existing in self.discovered_secrets:
            if self._secret_dedup_key(existing) == key:
                return False
        self.discovered_secrets.append(secret)
        return True

    @staticmethod
    def _secret_context(text: str, start: int, end: int, window: int = 120) -> str:
        snippet_start = max(0, start - window)
        snippet_end = min(len(text), end + window)
        snippet = text[snippet_start:snippet_end]
        return snippet.strip()[:2000]

    def _extract_secrets_from_response(self, content: str, source_url: str, content_type: str = '') -> List[Dict]:
        """Detect exposed credentials in any HTTP response body (JSON, HTML, text, JS)."""
        ...
        def add_finding(secret_type: str, name: str, context: str, severity: str = 'critical',
                        match_start: int = None, match_end: int = None) -> None:
            key = (source_url, secret_type, name.lower())
            if key in seen_keys:
                return
            seen_keys.add(key)
            entry = {
        ...
        def _looks_like_pem_key_block(block: str) -> bool:
            # Extract "base64-ish" chars between markers and require enough payload.
            inner = re.sub(
        ...
        for match in self._PEM_PRIVATE_KEY.finditer(content):
            block = match.group(0) if hasattr(match, "group") else ""
            if not _looks_like_pem_key_block(block):
                continue
            add_finding(
                'private_key',
                'PEM private key',
                self._secret_context(content, match.start(), match.end()),
                match_start=match.start(),
                match_end=match.end(),
            )

        gcp_pk = self._GCP_PRIVATE_KEY_JSON.search(content)
        if gcp_pk and not any(r['type'] == 'private_key' for r in results):
            add_finding(
                'private_key',
                'JSON private_key field (PEM)',
                self._secret_context(content, gcp_pk.start(), gcp_pk.end(), window=200),
                match_start=gcp_pk.start(),
                match_end=gcp_pk.end(),
            )

        has_service_account_type = bool(self._SERVICE_ACCOUNT_TYPE.search(content))
        email_match = self._SERVICE_ACCOUNT_EMAIL.search(content)
        client_email = ''
        if email_match:
            client_email = (email_match.group(1) or email_match.group(2) or '').strip()
        has_private_key = bool(self._PEM_PRIVATE_KEY.search(content) or self._GCP_PRIVATE_KEY_JSON.search(content))

        sa_type = self._SERVICE_ACCOUNT_TYPE.search(content)
        if has_service_account_type and (has_private_key or client_email):
            sa_name = client_email or 'Google service account'
            ctx_start = sa_type.start() if sa_type else 0
            ctx_end = sa_type.end() if sa_type else len(content)
            add_finding(
                'service_account',
                sa_name,
                self._secret_context(content, ctx_start, ctx_end, window=400),
                match_start=ctx_start,
                match_end=min(len(content), ctx_end + 400),
            )
        elif client_email and '@' in client_email and (
            'gserviceaccount.com' in client_email.lower()
            or has_private_key
        ):
            em = email_match
            add_finding(
                ...
                self._secret_context(
                    content,
                    em.start() if em else 0,
                    em.end() if em else len(content),
                    window=400,
                ),
                match_start=em.start() if em else None,
                match_end=em.end() if em else None,
            )

        for match in self._AWS_ACCESS_KEY_ID.finditer(content):
            add_finding(
                'aws_access_key',
                match.group(1),
                self._secret_context(content, match.start(), match.end()),
        ...
        aws_sk = self._AWS_SECRET_KEY_JSON.search(content)
        if aws_sk:
            add_finding(
                'aws_secret_key',
                'AWS secret access key',
                self._secret_context(content, aws_sk.start(), aws_sk.end(), window=120),
                severity='critical',
                match_start=aws_sk.start(),
                match_end=aws_sk.end(),
            )
        ...
        for pattern, secret_type in patterns:
            for m in re.finditer(pattern, js, re.IGNORECASE | re.MULTILINE):
                try:
                    ...
                    context = js[start:end].strip()[:500]
                    results.append({
                        'type': secret_type,
                        'name': name,
                        'context': context,
    ...
    def extract(self, flow, detected_technologies: Dict = None) -> Dict[str, List[str]]:
        ...
        try:
            ...
            capture_flow_id = getattr(flow, 'id', None)
            if capture_flow_id and content:
                self._cache_response_body_for_flow(capture_flow_id, content)

            def _attach_flow_id(secret: Dict) -> Dict:
                if capture_flow_id:
                    secret['flow_id'] = capture_flow_id
                    if capture_flow_id in self.flow_response_bodies:
                        secret['response_body_b64'] = self.flow_response_bodies[capture_flow_id]
                return secret
            ...
            if 'javascript' in content_type or 'application/javascript' in content_type or '.js' in flow.request.path:
                ...
                if base_url not in self.analyzed_js_files:
                    ...
                    try:
                        ...
                        for s in secrets:
                            s.setdefault('location', 'javascript')
                            s.setdefault('severity', 'medium')
                            self._register_discovered_secret(_attach_flow_id(s))
                    except Exception as e:
            ...
            # Passive credential / secret scan on any response body
            try:
                response_secrets = self._extract_secrets_from_response(content, base_url, content_type)
                endpoints['response_secrets'] = response_secrets
                for secret in response_secrets:
                    self._register_discovered_secret(_attach_flow_id(secret))
                if response_secrets:
                    print(
                        f"[SECRET DETECTION] Found {len(response_secrets)} exposed secret(s) "
                        f"in {base_url}: {[s.get('type') for s in response_secrets]}"
                    )
            except Exception as e:
                print(f"[ENDPOINT EXTRACTION] Error extracting response secrets: {e}")
                endpoints['response_secrets'] = []
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        for s in secrets:
                            s.setdefault('location', 'javascript')
                            s.setdefault('severity', 'medium')
                            self._register_discovered_secret(s)
                        for s in self._extract_secrets_from_response(js_content, js_url, 'application/javascript'):
                            self._register_discovered_secret(s)
                    except Exception as e:
